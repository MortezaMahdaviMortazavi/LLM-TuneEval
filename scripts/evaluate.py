...
from args import evaluate_arguments
import warnings
...
class Preprocessor:
    ...
    def _convert_non_answers(self, target):
        actual_out = "پاسخ در متن وجود ندارد"
...
def get_metric_objects(metric_names, tokenizer, device):
    """
...
def main():
    ...
    metrics = get_metric_objects(args.metrics, tokenizer, args.device)
    evaluator = Evaluator(task=args.task, metrics=metrics, logfile=args.log_path)

    references = df[args.target_column].tolist()
    predictions = pred[args.predict_column].tolist()
