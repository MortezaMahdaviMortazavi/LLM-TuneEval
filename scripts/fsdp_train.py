...
from args import fsdp_train_arguments
from utils import load_datasets
from callbacks import HardwareUtilizationCallback , SpeedMonitoringCallback
...
def training_function(args):
    ...
    quant_storage_dtype = torch.bfloat16 #if args.bf16 else torch.float32
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        use_cache=False if args.fsdp_activation_checkpointing else True,
    )
    ...
    training_args = TrainingArguments(
        ...
        per_device_train_batch_size=args.per_device_train_batch_size,  # Batch size per device (e.g., per GPU) during training.
        per_device_eval_batch_size=args.per_device_eval_batch_size, # Batch size per device during training
        gradient_accumulation_steps=args.gradient_accumulation_steps,  # Number of forward passes before performing a backward/update pass.
        ...
        fsdp=args.fsdp,  # FSDP configuration to shard model parameters across GPUs.
        fsdp_config={  # Detailed configuration for FSDP.
            'backward_prefetch': "backward_pre",  # Prefetch strategy for backward pass to improve efficiency.
            'forward_prefetch': True, #args.fsdp_forward_prefetch,  # Prefetch strategy for forward pass (if needed).
            'use_orig_params': True, #args.use_orig_params,  # Use original (unsharded) parameters during training.
            'activation_checkpointing': args.fsdp_activation_checkpointing,  # Save memory by recomputing activations during backward pass.
        },
    ...
    print("Adding smonitoring callback...")
    callbacks = [
        SpeedMonitoringCallback(args.logging_steps)
    ]
    ...
    try:
        ...
        if trainer.is_fsdp_enabled:
            trainer.accelerator.state.fsdp_plugin.flatten_parameters = True
            trainer.accelerator.state.fsdp_plugin.set_state_dict_type("FULL_STATE_DICT")
            print("FSDP parameters flattened for saving.")
